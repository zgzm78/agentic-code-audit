...
RUN mkdir -p .tools/bin \
    ...
    && curl --fail --location --show-error "https://github.com/google/osv-scanner/releases/download/v${OSV_SCANNER_VERSION}/osv-scanner_linux_amd64" -o .tools/bin/osv-scanner \
    && curl --fail --location --show-error "https://github.com/aquasecurity/trivy/releases/download/v${TRIVY_VERSION}/trivy_${TRIVY_VERSION}_Linux-64bit.tar.gz" -o /tmp/trivy.tar.gz \
    && tar -xzf /tmp/trivy.tar.gz -C .tools/bin trivy \
    && chmod +x .tools/bin/trivy \
    && curl --fail --location --show-error "https://download.docker.com/linux/static/stable/x86_64/docker-${DOCKER_CLI_VERSION}.tgz" -o /tmp/docker.tgz \
