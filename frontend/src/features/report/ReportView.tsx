...
export default function ReportView({ data }: { data: AuditData }) {
  ...
  return <div className="view report-view">
    ...
    <section className="report-layout"><aside className="report-outline"><span className="eyebrow">REPORT INDEX</span>{headings.length ? headings.slice(0, 16).map((heading) => <a key={`${heading.id}-${heading.label}`} href={`#${heading.id}`} onClick={(event) => scrollToHeading(event, heading.id)} className={`depth-${heading.depth}`}>{heading.label}</a>) : <span className="outline-empty">报告生成后显示章节目录</span>}</aside><article className="report-paper" id="raw">{data.report ? <ReactMarkdown remarkPlugins={[remarkGfm]} skipHtml components={markdownComponents}>{data.report}</ReactMarkdown> : <div className="report-placeholder"><FileText size={38} /><h3>报告尚未生成</h3><p>任务完成后，报告会以结构化阅读模式呈现。</p></div>}</article></section>
  </div>;
}
...
function createMarkdownComponents(headings: Heading[]) {
  const idsByLabel = new Map<string, string[]>();
  for (const heading of headings) {
    const ids = idsByLabel.get(heading.label) || [];
    ids.push(heading.id);
    idsByLabel.set(heading.label, ids);
  }
  const idFor = (children: unknown) => {
    const label = normalizeHeadingLabel(textOf(children));
    const ids = idsByLabel.get(label);
    return ids?.shift() || slugify(label);
  };
  return {
    h1: ({ children }: { children?: unknown }) => <h1 id={idFor(children)}>{children}</h1>,
    h2: ({ children }: { children?: unknown }) => <h2 id={idFor(children)}>{children}</h2>,
    h3: ({ children }: { children?: unknown }) => <h3 id={idFor(children)}>{children}</h3>,
    a: ({ href, children }: { href?: string; children?: unknown }) => <a href={href} target={href?.startsWith("http") ? "_blank" : undefined} rel="noreferrer">{children}</a>,
  };
}
